...
pub(super) fn postgres_column_definition(column: &TableColumnSchema) -> IpcResult<String> {
    ...
    if let Some(generated) = column.generated.as_ref() {
        if generated.storage != crate::engine::types::TableGeneratedColumnStorage::Stored {
            return Err(IpcError::validation_failed(
                "PostgreSQL 生成列只支持 STORED",
            ));
        }
    ...
    } else if column.is_identity || column.identity.is_some() {
        if normalized_optional_text(column.default_value.as_deref()).is_some() {
            return Err(IpcError::validation_failed("自增列不能同时设置默认值"));
        }
        let generation = column
            .identity
            .as_ref()
            .map(|identity| identity.generation.clone())
            .unwrap_or(crate::engine::types::TableIdentityGeneration::ByDefault);
        match generation {
            crate::engine::types::TableIdentityGeneration::Always => {
                definition.push_str(" GENERATED ALWAYS AS IDENTITY")
            }
            crate::engine::types::TableIdentityGeneration::ByDefault => {
    ...
    if column.is_primary_key || column.is_identity || !column.nullable {
...
pub(super) fn postgres_table_constraint_definition(
    ...
    match constraint.kind {
        TableConstraintKind::ForeignKey => {
            ...
            let referenced_table = postgres_referenced_table_name(
                foreign_key.schema_name.as_deref(),
                &foreign_key.table_name,
            )?;
            let mut definition = format!(
                "CONSTRAINT {} FOREIGN KEY ({}) REFERENCES {} ({})",
                quote_pg_identifier(&name),
                quote_column_list(&constraint.columns, quote_pg_identifier),
                referenced_table,
                quote_column_list(&foreign_key.columns, quote_pg_identifier)
            );
            if let Some(action) = foreign_key.on_update.as_ref() {
                definition.push_str(" ON UPDATE ");
                definition.push_str(postgres_referential_action(action));
            }
        ...
        TableConstraintKind::PrimaryKey | TableConstraintKind::Unique => Err(
            IpcError::validation_failed("该约束类型不应通过 PostgreSQL advanced renderer 渲染"),
        ),
    }
}

pub(super) fn postgres_drop_constraint_statement(
    table_name: &str,
    constraint: &TableConstraintSchema,
) -> IpcResult<String> {
    let name = normalized_non_empty_identifier(&constraint.name, "约束名")?;
...
pub(super) fn postgres_identity_change_statements(
    table_name: &str,
    change: &ColumnIdentityChange,
) -> IpcResult<Vec<String>> {
    let column_name = normalized_non_empty_identifier(&change.column_name, "列名")?;
    let column = quote_pg_identifier(&column_name);
    match (&change.old_identity, &change.new_identity) {
        (None, Some(identity)) => Ok(vec![format!(
            "ALTER TABLE {table_name} ALTER COLUMN {column} ADD {} AS IDENTITY",
            postgres_identity_generation(&identity.generation)
        )]),
        (Some(_), None) => Ok(vec![format!(
            "ALTER TABLE {table_name} ALTER COLUMN {column} DROP IDENTITY"
        )]),
        (Some(old_identity), Some(new_identity))
            if old_identity.generation != new_identity.generation =>
        {
            Ok(vec![format!(
                "ALTER TABLE {table_name} ALTER COLUMN {column} SET GENERATED {}",
                postgres_identity_generation_keyword(&new_identity.generation)
            )])
        }
        (Some(_), Some(_)) => Err(IpcError::validation_failed(format!(
            "暂不支持修改 identity 序列参数 '{}'",
            change.column_name
        ))),
...
pub(super) fn postgres_identity_generation(
    generation: &crate::engine::types::TableIdentityGeneration,
) -> &'static str {
    match generation {
        crate::engine::types::TableIdentityGeneration::Always => "GENERATED ALWAYS",
        crate::engine::types::TableIdentityGeneration::ByDefault => "GENERATED BY DEFAULT",
    }
}

pub(super) fn postgres_identity_generation_keyword(
    generation: &crate::engine::types::TableIdentityGeneration,
) -> &'static str {
    match generation {
        crate::engine::types::TableIdentityGeneration::Always => "ALWAYS",
        crate::engine::types::TableIdentityGeneration::ByDefault => "BY DEFAULT",
    }
}

pub(super) fn postgres_referenced_table_name(
    schema: Option<&str>,
    table: &str,
) -> IpcResult<String> {
    let table = normalized_non_empty_identifier(table, "引用表")?;
    match normalized_optional_text(schema) {
        Some(schema) => Ok(format!(
            "{}.{}",
            quote_pg_identifier(&schema),
            quote_pg_identifier(&table)
        )),
        None => Ok(quote_pg_identifier(&table)),
    }
}

pub(super) fn postgres_referential_action(
...
pub(super) fn postgres_parse_referential_action(
    value: Option<&str>,
) -> Option<TableReferentialAction> {
    match value?.trim() {
        "a" => Some(TableReferentialAction::NoAction),
        "r" => Some(TableReferentialAction::Restrict),
        "c" => Some(TableReferentialAction::Cascade),
        "n" => Some(TableReferentialAction::SetNull),
        "d" => Some(TableReferentialAction::SetDefault),
        _ => None,
    }
}
...
pub(super) fn ensure_schema_matches_postgres_table_parts(
    schema: &TableSchema,
    database: &str,
    schema_name: &str,
    table: &str,
    ...
    if schema.basics.database_name.trim() == database
        && schema.basics.schema_name.trim() == schema_name
