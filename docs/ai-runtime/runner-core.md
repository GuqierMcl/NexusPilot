...
每个模型请求先估算目标 Provider/model 的窗口和输出预留。能安全容纳时优先发送完整 active raw lineage；否则才使用经 ancestry、parent、cursor、prefix hash、format/compatibility 校验的 provider-neutral checkpoint，再追加完整 raw tail 和确定性的 Safety State。除了完整终态 Run，当前 Run 中以 `StepStartPart … StepFinishPart` 封闭、ToolPart/ToolCall 终态配对且没有 pending Permission 的 model-step prefix 也可成为 `sealed_step` coverage；开放 stream、开放 block、运行中工具或待决审批一律阻断。`completed` Run 的 output identity 必须与 Assistant parts 一致；失败路径按设计可保留已经聚合的 Assistant/工具事实而不写 `Run.output`，因此 `failed` Run 在 Assistant、ToolPart/ToolCall 全部终态且没有 pending Permission 时仍可被后续 checkpoint 覆盖，不能永久阻断后续压缩。AI SDK 7 调用边界严格分离两个通道：assembled agent prompt、provider-neutral checkpoint 与 Runtime/Safety State 作为 system-level `instructions`，raw user/assistant/tool history 与当前 ToolLoop suffix 作为不含 system role 的 `messages`；rolling summary 同样把 summary prompt、父 checkpoint memory 与 Safety State 放入 `instructions`，并在每次 source chunk 后追加一条 Runtime-owned user generation request。这个终端请求让以历史 Assistant 结束的 OpenAI-compatible prompt 明确进入“现在生成 checkpoint”状态；它参与 summary input budget 与 usage 估算，但不进入 raw transcript 或持久化 Message。Runtime 在初始调用和每次 `prepareStep` 重建后都拒绝 system message 泄漏到 `messages`。retained suffix 的内容 hash 与起始 request index 进入 plan identity；checkpoint 只替换 cursor 已覆盖的 suffix，cursor 后的新 step 保持 exact replay。planner 将实际考虑但拒绝的 checkpoint ID 与稳定原因写入 `ContextPlan.checkpointRejections`，供 `context.plan.created` 审计；不写 summary、Safety State body 或任意 metadata。checkpoint、plan、usage 与 Activity 都是派生、append-only 事实，不能替代或改写 transcript。checkpoint 与 checkpoint event、plan 与 plan event 各自在一个事务中提交；checkpoint 与对应 Activity `created` 也在同一事务中收敛。写入由带 CAS/fencing claim 的 `ContextCompactionService` 统一完成，CAS/revalidation 拒绝另写脱敏 durable diagnostic。该 service 同时支持自动与手动入口的 `trigger: "manual"`；手动入口已实现为面向用户的 `/compact` 会话操作，由 `GET/POST /v1/conversations/:conversationId/compactions` 与 `POST /v1/conversations/:conversationId/compactions/:operationId/cancel` 承载，仍通过同一 checkpoint service 编排。

自动压缩在新 Run 的首次请求前和同一 Run 每个后续 ToolLoop/Permission continuation 请求前执行；达到 soft threshold 后，它是必须成功的 gate。首次请求触发 `auto_pre_turn`；已封闭 step 后触发 `auto_mid_turn`，成功后同一 Run 立即 continuation，不需要新的用户消息。summary Provider/校验失败会立即终结当前 Run：pre-turn 时首次主模型调用次数为零，mid-turn 时下一主模型调用次数为零，而此前 sealed step、工具结果和 usage 保持不变；两种情况都不保存 checkpoint，也不回退到 raw history。只有第一次结果严格满足“空 text、length finish、reasoning tokens 大于零且 text tokens 明确为零”时，Runtime 才在模型 output limit 内扩大预算并缩小 summary input chunk，最多重试一次；reasoning 正文永远不作为 checkpoint。durable `ContextCompactionActivity` 以同一 identity 表达 `preparing/created/failed/recovered/interrupted`，并作为独立 `data-context-compaction` timeline part 投影在真实 request boundary；它不属于 User/Assistant 可编辑正文，也不进入 Provider prompt。EventBus invalidation + Snapshot 可在 summary 静默阶段显示 `preparing`，而同一 `/v1/runs` UI stream 必须在下一 model-step 输出前从 Store 注入同 identity 的 durable 终态。这样后续 token 更新的是 AI SDK 自己的 active streaming message，不会把已显示的 divider 覆盖到 Run 结束后才恢复；stream 和最终 Snapshot 都依靠 `cmp_*` 原位 upsert。旧 footer marker 仅为兼容读取。

明确的 Provider context-overflow 只有在失败 attempt 尚未输出、调用工具、请求/消费 Permission 或产生副作用时，才可用同一 Provider/model 安全恢复一次；已经持久化的更早 sealed steps 保留且不重放。初始 overflow 规划不会盲目复用可能已被 Provider 拒绝的旧 checkpoint；只有本次 recovery preparation 通过 CAS 新创建的 checkpoint 能作为 replacement attempt 的强制候选。其余错误、不安全事实或第二次失败都保留原始 Provider/AI SDK 错误，不自动换模型。替代请求开始前的 `context.overflow.retrying` 仅保留首次错误审计，只有替代请求的 finish 边界才追加 `context.overflow.recovered` 并把同一 Activity 从 `created` 提升为 `recovered`；因此第二次构造或执行失败不会向 UI 投影 `recovered`。AI SDK 在 error 后产生的无语义 `onStepEnd` 只会被丢弃，不能把已通过无输出 gate 的 recovery 误判为新事实；任何真实 text/reasoning/source/tool/Permission 事实仍立即禁止 retry。`Run.usage` 仍是整次 Run 的累计计费 usage；每个模型请求另持久化 `ContextUsage`。请求 estimate、Provider observation 和 output reserve 分别保留；Assistant 完成落盘后，Runtime 重新估算包含最新 Assistant 的 `nextTurnForecast` 并持久化。主上下文百分比始终使用 forecast estimate，Provider actual input 和 reserve 只作辅助信息，因此请求开始、Provider observation、step end 和 Run finish 不会因口径切换而反向跳动。summary 生成若包含多次 rolling 调用，则把本轮每次调用的估算聚合进新 checkpoint 自带的 `ContextUsage`，即使 Provider 没有返回 usage 也保留估算；Provider 实际返回的标量进入可选 `providerObservation`，未返回的单个标量保持缺失而非零，所有标量均缺失时省略 observation。父 checkpoint usage 不继承，且 summary usage 不进入累计 `Run.usage`。完整行为、格式与演进约束见 [context-compaction specification](../comet/specs/agent-context-compaction/spec.md)。
...
- 核心 HTTP/SSE/WebSocket transport 不设置短 idle timeout 或累计连接寿命；合法的长模型生成、上下文压缩、工具执行和审批等待由业务 timeout、显式 interrupt、shutdown 与 heartbeat/liveness 分别控制。
- AI SDK `abort` part 的 reason 必须保留；已确定的 Provider/AI SDK failure 不得被稍后的 transport abort 覆盖成通用 `client_disconnect`。
