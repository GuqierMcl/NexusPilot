...
import { projectActiveTabContext, readActiveTabPart, type ActiveTabContext } from "../../shared/active-tab-context";
...
const body = { response_mode: "stream", model: { provider_id: "test", model_id: "test" }, input: { parts: [{ type: "text" as const, text: "帮我理解这个页面" }] }, activeTabContext: snapshot };
...
test("snapshot survives SQLite reload, UI projection, history editing and model projection", async () => {
  ...
  try {
    ...
    expect(model).toEqual([{ role: "user", content: [body.input.parts[0], { type: "text", text: projectActiveTabContext(snapshot) }] }]);
    const without = { ...first.userMessage, activeTabContext: undefined };
    expect(estimateMessageTokens(restored) - estimateMessageTokens(without)).toBe(2 + estimateTextTokens(projectActiveTabContext(snapshot)));
    ...
    const removed = runner.start({ ...parsed.runRequest, activeTabContext: undefined, conversationId: first.conversation.id, replaceFromMessageId: edit.userMessage.id });
    expect(removed.userMessage.activeTabContext).toBeUndefined();
    expect(store.getMessage(first.userMessage.id)).toEqual(restored);
  } finally { db.close(); }
});

test("invalid tab metadata is rejected before Run creation, including mixed files", async () => {
  const db = openRuntimeDatabase(":memory:");
  try {
    const store = new RuntimeSqliteStore(db);
    const app = runRoutes({ runtimeStore: store, providerService: null });
    for (const altered of [{ version: 2 }, { type: "unregistered" }, { title: "x".repeat(257) }, { sqlText: "SELECT secret" }, { capabilities: { ...snapshot.capabilities, content: true } }]) {
      const value = { ...body, activeTabContext: { ...snapshot, ...altered }, input: { parts: [...body.input.parts, { type: "file", attachment_id: "att_missing" }] } };
