...
function getDatabaseContainer(node: ExplorerTreeNode | null) {
...
export function DeleteDatabaseDialog({
    ...
    const dropOperation = driverConfig?.dropDatabase;
    ...
    useEffect(() => {
        if (open && connection && node && dropOperation) {
    ...
    }, [connection, dropOperation, node, open]);
    ...
    useEffect(() => {
        ...
        try {
            target = getDatabaseContainer(node);
        } catch (error) {
            console.error("[explorer] build drop database preview input failed", error);
            setPreviewStatements([]);
            return;
        }
        ...
        dropOperation.preview(connection.id, target)
            ...
            .catch((error) => {
                if (previewRequestIdRef.current === requestId) {
                    console.error("[explorer] preview drop database failed", error);
                    setPreviewStatements([]);
                }
            })
            .finally(() => {
    ...
    async function handleFinalDelete() {
        ...
        try {
            const target = getDatabaseContainer(node);
            ...
            toast.success(`数据库“${applied.name}”已删除`);
            onOpenChange(false);
            onSuccess(applied);
        } catch (error) {
            console.error("[explorer] drop database failed", error);
                ...
                <DialogContent className="sm:max-w-md" showCloseButton>
                    ...
                    <div className="flex flex-col gap-4">
                        <p className="text-sm text-destructive">
                            此操作会删除数据库“{databaseName}”及其全部对象，无法恢复。
                        </p>
