...
function toBaseToastTimeout(duration: number | undefined): number | undefined {
...
function addToast(
  ...
  return toastManager.add({
    ...
    timeout: toBaseToastTimeout(options?.duration),
...
const toast = {
  success: (title: React.ReactNode, options?: AppToastOptions): string =>
    addToast("success", title, options),
  error: (title: React.ReactNode, options?: AppToastOptions): string =>
    addToast("error", title, options),
  info: (title: React.ReactNode, options?: AppToastOptions): string =>
    addToast("info", title, options),
  warning: (title: React.ReactNode, options?: AppToastOptions): string =>
...
function ToastViewport({
  ...
  return (
    <ToastPrimitive.Viewport
      ...
      className={cn(
        "pointer-events-none fixed right-4 bottom-4 z-[60] w-full max-w-sm outline-none",
        className,
      )}
      {...props}
    />
...
function ToastIcon({ type }: { type: string | undefined }) {
  ...
  if (type === "success") return <CircleCheckIcon className={className} aria-hidden="true" />
  if (type === "info") return <InfoIcon className={className} aria-hidden="true" />
  if (type === "warning") return <TriangleAlertIcon className={className} aria-hidden="true" />
  if (type === "error") return <OctagonXIcon className={cn(className, "text-destructive")} aria-hidden="true" />
...
function Toaster({ timeout, limit }: ToasterProps) {
  return (
