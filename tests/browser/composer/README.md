...
Run `node tests/browser/composer/active-tab-check.cjs` against the same fixture to check selected-tab metadata, draft omission and reset, native message copying, historical revisions and editing, failed-draft recovery, snapshot freezing during attachment uploads, and coexistence with references and commands. It creates 100 in-memory tabs and asserts that only one snapshot enters each request. `ACTIVE_TAB_SCREENSHOT` optionally saves a narrow dark-theme screenshot to a specified file in an existing directory.
